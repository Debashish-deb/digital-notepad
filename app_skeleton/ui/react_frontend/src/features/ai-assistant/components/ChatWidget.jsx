...
import { useChatAutoScroll } from '@/shared/hooks/useChatAutoScroll.js';
import { revealTextProgressively } from '@/lib/chatMotion.js';
import {
...
export default function ChatWidget({
  ...
  const resizeTextarea = useCallback(() => {
    ...
    if (!el) return;
    const minPx = Number.parseFloat(getComputedStyle(el).minHeight) || 48;
    el.style.height = 'auto';
    el.style.height = `${Math.max(minPx, Math.min(el.scrollHeight, 160))}px`;
  }, []);
  ...
  const userMessageScrollKey = useMemo(() => {
  ...
  const { containerRef: messagesContainerRef, endRef: messagesEndRef } = useChatAutoScroll(
    ...
    {
      behavior: isStreaming || loading ? 'auto' : 'smooth',
      forceKey: userMessageScrollKey,
    },
  );
