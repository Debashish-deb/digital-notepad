...
from app_skeleton.api.agent_orchestrator.orchestrator import run_category_chat
...
from app_skeleton.api.agent_orchestrator.trace_store import get_trace
from app_skeleton.api.routers.chat import _chat_llm, _search_service
from app_skeleton.api.common import rag_agent
...
def chat_category(
    ...
    def rag_fn(message, *, project_codes, user, provider, model, retrieval_only=True):
        del user, provider, model, retrieval_only
    ...
    def llm_factory(provider: str | None, model: str | None):
        if req.use_local_models and not provider:
            return _chat_llm("ollama", model)
        return _chat_llm(provider, model)

    return run_category_chat(
        req.message,
