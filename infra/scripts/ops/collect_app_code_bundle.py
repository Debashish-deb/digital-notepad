...
APP_CODE_ROOTS: tuple[str, ...] = (
    "omeia/api",
    "omeia/security",
    "omeia/ui/react_frontend/src",
    "omeia/ui/react_frontend/scripts",
...
EXCLUDE_PATH_PREFIXES: tuple[str, ...] = (
    "omeia/data/",
    "omeia/ui/react_frontend/public/processed/",
    "omeia/ui/react_frontend/public/database/",
    "omeia/ui/react_frontend/node_modules/",
    "configs/.env",
    "configs/secrets/",
)
...
def _categorize(rel: str) -> str:
    ...
    if norm.startswith("omeia/api/"):
        return "api"
    if norm.startswith("omeia/security/"):
        return "security"
    ...
    if norm.startswith("tests/"):
        return "tests"
    if norm.startswith("omeia/pipelines/"):
        return "pipelines"
    return "other"
...
def main() -> int:
    ...
    parser.add_argument(
        "--include-pipelines",
        action="store_true",
        help="Include omeia/pipelines (excludes vendored deepcell-tf)",
    )
    parser.add_argument(
        "--max-file-kb",
        type=int,
