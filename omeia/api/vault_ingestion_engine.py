...
from omeia.api.paths import BLUEPRINT_ROOT, DATABASE_ROOT, PROJECTS_ROOT
from omeia.api.qdrant_collections import VAULT_CHUNKS as VAULT_QDRANT_COLLECTION
...
from omeia.api.data_layout import ingestion_report_write_dir, iter_ingestion_report_files
...
def _maybe_knowledge_index(
    ...
    *,
    ...
    if not result.chunks:
        return
    try:
        from omeia.api.platform_flags import knowledge_indexer_enabled

        if not knowledge_indexer_enabled():
            return
        from omeia.api.knowledge_indexer import index_vault_extraction
...
def _maybe_vectorize(cur, asset_id: str, result: de.ExtractionResult) -> None:
    ...
    try:
        from omeia.api.platform_flags import vectorization_enabled, vault_use_vector_indexer_enabled
        ...
        if use_vector_indexer:
            ...
            from omeia.api.qdrant_vectors import upsert_text_points
