...
def build_research_nav(hit: dict[str, Any]) -> dict[str, Any]:
    ...
    if source_type == "dataset":
        return {"main": "research_knowledge", "sub": "datasets", "dataset_accession": hit.get("dataset_accession")}
    if source_type in {"publication", "preprint", "publication_metadata"}:
        return {"main": "research_knowledge", "sub": "publications", "source_url": hit.get("source_url"), "doi": hit.get("doi")}
    return {"main": "research_knowledge", "sub": "sources", "source_url": hit.get("source_url")}
