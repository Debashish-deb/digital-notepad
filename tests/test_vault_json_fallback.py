...
def test_search_vault_skips_json_when_fallback_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("app_skeleton.api.raw_vault_store._search_vault_postgres", return_value=[]):
        with patch("app_skeleton.api.raw_vault_store._search_vault_json") as mock_json:
...
def test_search_vault_uses_json_when_fallback_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("app_skeleton.api.raw_vault_store._search_vault_postgres", return_value=[]):
        with patch(
            "app_skeleton.api.raw_vault_store._search_vault_json",
            return_value=[{"asset_id": "a1", "filename": "f.txt"}],
        ) as mock_json:
            with patch(
                "app_skeleton.api.raw_vault_store._sanitize_metadata_in_rows",
