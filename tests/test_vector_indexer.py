...
from omeia.api.vector_indexer import ChunkRecord, build_point_from_chunk, upsert_text_chunks


def test_build_point_from_chunk_uses_embed_text():
    with patch("omeia.api.vector_indexer.embed_text", return_value=[0.1, 0.2, 0.3]) as mock_embed:
...
def test_upsert_text_chunks_skips_short_text():
    client = MagicMock()
    with patch("omeia.api.vector_indexer.upsert_text_points", return_value=0) as mock_upsert:
...
def test_upsert_text_chunks_batches_to_qdrant():
    ...
    with patch("omeia.api.vector_indexer.embed_text", return_value=[0.5] * 384):
        with patch("omeia.api.vector_indexer.upsert_text_points", return_value=1) as mock_upsert:
