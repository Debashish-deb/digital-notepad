...
def main():
    ...
    # We will keep the top-level lifespan and app creation in main.py
    # Everything before @app.get("/api/billing-instructions") (line 565) except app creation
    # is considered common. But parsing AST is cleaner.
    ...
    for node in tree.body:
        ...
        if is_route:
            ...
            # Using regex to replace just the decorator
            src = re.sub(r'@app\.', '@router.', src)
            routes_by_path.append((route_path, src))
        else:
            # We skip 'app = FastAPI(...)' and CORS etc because they stay in main.
    ...
    for path, src in routes_by_path:
        ...
        elif path.startswith("/projects") or path.startswith("/notebook") or path.startswith("/decisions") or path.startswith("/tasks") or path.startswith("/auto_logs") or path.startswith("/team") or path.startswith("/folders") or path.startswith("/datasets") or path.startswith("/pipeline_runs") or path.startswith("/checklists") or path.startswith("/platform/search") or path.startswith("/infrastructure") or path.startswith("/ai-models") or path.startswith("/publications"):
            routers["research"].append(src)
        elif path.startswith("/ask") or path.startswith("/install_guide") or path.startswith("/lumi_job") or path.startswith("/parse_log") or path.startswith("/run_checker") or path.startswith("/clinical") or path.startswith("/features") or path.startswith("/analysis-runs"):
