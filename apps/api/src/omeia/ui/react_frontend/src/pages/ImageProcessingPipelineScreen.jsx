...
import { HubSectionFrame } from '@/shared/layout/HubNestedNav.jsx';
import { LazyViewFallback } from '@/shared/ui/common/LazyViewFallback.jsx';

const Pipeline3DScene = lazy(() =>
  import('@/features/computational/components/Pipeline3DScene.jsx').then((m) => ({ default: m.Pipeline3DScene })),
);
const RunPipelineTab = lazy(() => import('@/features/projects/components/RunPipelineTab.jsx'));
import {
  PIPELINE_ARTIFACTS,
    ...
    <div className="ipp-guide">
      ...
      <div className="ipp-content-grid">
        ...
        <InfoCard icon={FolderOpen} title="Bundled in this app">
          <p>Full pipeline source lives at <code>omeia/pipelines/lumi_image_processing/</code> — synced from the lab CSC scripts tree.</p>
        </InfoCard>
      </div>
    </div>
  );
          ...
          <div className="ipp-artifact-list">
            {PIPELINE_ARTIFACTS.map((a) => (
              <span key={a.path} className="ipp-artifact-link text-footnote">
                omeia/pipelines/lumi_image_processing/{a.path}
              </span>
            ))}
          </div>
        </InfoCard>
