...
if [[ -x "$ROOT/scripts/generate_ollama_token.sh" ]] && ! grep -q '^OLLAMA_INTERNAL_TOKEN=.\+' "$ROOT/configs/.env" 2>/dev/null; then
  ...
  "$ROOT/scripts/generate_ollama_token.sh"
fi

if [[ -x "$ROOT/scripts/setup_ollama_local_llm.sh" ]]; then
  env -u OLLAMA_RESEARCH_MODELS -u OLLAMA_MODELS "$ROOT/scripts/setup_ollama_local_llm.sh"
fi

# Resume any catalog pulls interrupted earlier (medllama2, meditron, etc.)
if [[ -x "$ROOT/scripts/pull_ollama_research_models.sh" ]]; then
  echo "=== Resuming catalog model pulls (background) ==="
  env -u OLLAMA_RESEARCH_MODELS -u OLLAMA_MODELS nohup "$ROOT/scripts/pull_ollama_research_models.sh" &
...
echo "  OLLAMA_BASE_URL=http://<this-host-ip>:11434/v1   # or use scripts/ollama_ssh_tunnel.sh"
