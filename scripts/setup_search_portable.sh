...
if [[ -f "$ROOT/configs/qdrant_collections.yaml" ]]; then
  ...
  python3 "$ROOT/scripts/create_qdrant_collections.py" || echo "    (Qdrant offline — semantic search will use Postgres/JSON fallback)"
fi
...
if [[ -d "$DATABASE_ROOT" ]]; then
  python3 -m app_skeleton.api.database_processor --all --refresh 2>/dev/null || \
    python3 "$ROOT/app_skeleton/api/database_processor.py" --all --refresh 2>/dev/null || \
