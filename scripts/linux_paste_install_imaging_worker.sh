...
mkdir -p docker/imaging-worker app_skeleton/api scripts
...
COPY app_skeleton/api/imaging_capabilities.py /opt/omeia/imaging_capabilities.py
COPY docker/imaging-worker/healthcheck.py /opt/omeia/healthcheck.py
CMD ["python", "/opt/omeia/healthcheck.py"]
EOF

cat > app_skeleton/api/imaging_capabilities.py <<'PYEOF'
...
cat > scripts/build_imaging_worker.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
chmod +x scripts/build_imaging_worker.sh

echo "Files created:"
ls -la docker-compose.imaging.yml docker/imaging-worker/ scripts/build_imaging_worker.sh
