...
Add to configs/.env (server-side only):
  ...
  OLLAMA_INTERNAL_TOKEN=<run scripts/generate_ollama_token.sh once>
  LLM_FALLBACK_PROVIDERS=ollama,mock

Override pulls: OLLAMA_MODELS=tag1,tag2 scripts/setup_ollama_local_llm.sh

Restart API: ./start.sh
Eval: LLM_PROVIDER=ollama CHAT_LLM_PROVIDER=ollama OLLAMA_MODEL=qwen2.5:7b-instruct .venv/bin/python3 scripts/run_ai_lab_assistant_eval.py
