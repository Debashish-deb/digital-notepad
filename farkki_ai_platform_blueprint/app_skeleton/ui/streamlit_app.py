...
with tabs[1]:
    st.markdown("### 📁 **Project Portfolio & Digital Metadata Catalogs**")
    ...
    try:
        ...
        if r_proj.status_code == 200:
            ...
            # Select project code
            p_code = st.selectbox("Select Target Project", list(proj_dict.keys()))
            p_data = proj_dict[p_code]
            
            # Display project details
            st.markdown(f"#### 🧬 **{p_data['project_name']}**")
            
            col_p1, col_p2 = st.columns(2)
            with col_p1:
                st.markdown(f"**Research Question:** {p_data['research_question']}")
                st.markdown(f"**Type:** `{p_data['project_type']}`")
                st.markdown(f"**Priority:** `{p_data['priority'].upper()}`")
                st.markdown(f"**Ethics Reference:** `{p_data['ethics_approval_reference'] or 'N/A'}`")
            with col_p2:
                st.markdown(f"**Principal Investigator:** {p_data['principal_investigator']}")
                st.markdown(f"**Project Lead:** {p_data['project_lead']}")
                st.markdown(f"**Collaborators:** {', '.join(p_data['collaborators'])}")
                st.markdown(f"**Latest Activity:** {p_data['latest_update']}")
            
            # Edit metadata details
            with st.expander("📝 Edit Project Registry Fields (System of Record Logging)"):
                with st.form("edit_project_form"):
                    up_short_title = st.text_input("Short Title", p_data["project_short_title"])
                    up_question = st.text_area("Research Question", p_data["research_question"])
                    ...
                    up_ethics = st.text_input("Ethics Reference", p_data["ethics_approval_reference"])
                    up_blockers = st.text_area("Current Blockers", p_data["current_blockers"])
                    up_next_actions = st.text_area("Next Actions", p_data["next_actions"])
                    up_summary = st.text_area("Project Summary", p_data["project_summary"])
                    up_update = st.text_input("Latest Activity Update", p_data["latest_update"])
            ...
            # Data catalogs for this project
            sub_col1, sub_col2, sub_col3 = st.columns(3)
            with sub_col1:
                st.markdown("##### 📁 **Folders Catalog**")
                r_fold = requests.get(f"{API_URL}/folders", params={"project_code": p_code})
                if r_fold.status_code == 200 and r_fold.json():
                    st.dataframe(pd.DataFrame(r_fold.json()), width='stretch')
                else:
                    st.info("No registered folders for this project.")
            with sub_col2:
                st.markdown("##### 🧪 **Datasets Inventory**")
                r_data = requests.get(f"{API_URL}/datasets", params={"project_code": p_code})
                if r_data.status_code == 200 and r_data.json():
                    st.dataframe(pd.DataFrame(r_data.json()), width='stretch')
                else:
                    st.info("No registered datasets for this project.")
            with sub_col3:
                st.markdown("##### 🚀 **Pipeline Runs**")
                r_runs = requests.get(f"{API_URL}/pipeline_runs", params={"project_code": p_code})
                if r_runs.status_code == 200 and r_runs.json():
                    st.dataframe(pd.DataFrame(r_runs.json()), width='stretch')
                else:
        ...
        if checklists:
            ...
            for cat in categories:
                ...
                for item in cat_items:
                    ...
                    with col_chk:
                        is_checked = st.checkbox("Select", label_visibility="hidden", value=checked, key=chk_key)
                    with col_info:
