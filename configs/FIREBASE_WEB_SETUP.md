...
**Production deploy:** set `VITE_API_URL` and `VITE_FIREBASE_*` in `react_frontend/.env.production` before `npm run build` — see `configs/DEPLOYMENT_ENV.md` and `docs/26_PRODUCTION_DEPLOYMENT.md`. The SPA calls the FastAPI host at `VITE_API_URL` (`app_skeleton/ui/react_frontend/src/App.jsx`).
...
**Repo:** `app_skeleton/ui/react_frontend/src/config/firebase.js`
