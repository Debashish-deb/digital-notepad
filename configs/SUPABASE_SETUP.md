...
The platform builds this automatically when `SUPABASE_DB_PASSWORD` is set (see `app_skeleton/api/supabase_config.py`). Until then, `POSTGRES_CONN` keeps pointing at local Docker Postgres.
